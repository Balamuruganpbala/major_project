...
const getWeatherDetails = (cityName, latitude, longitude) => {
    ...
    fetch(WEATHER_API_URL).then(response => response.json()).then(data => {
        ...
        let fiveDaysForecast =[] ,fiveDaysForecast2 =[] 
        forecastArray.forEach(forecast => {
            const forecastDate = new Date(forecast.dt_txt).getDate();
            if (!uniqueForecastDays.has(forecastDate) && uniqueForecastDays.size < 8) {
            ...
            else if(y>0)
            {
                ...
                dt.setDate(forecastDate+8-y)
                fiveDaysForecast2.push(forecast);
                fiveDaysForecast2[fiveDaysForecast2.length-1].dt_txt=dt.toISOString().substring(0,10);
                y-=1
            }
        });
        fiveDaysForecast2.forEach(f => fiveDaysForecast.push(f));
